...
app = FastAPI(title="倍斯特Mock API", version="2.0.0", description="倍斯特测试数据框架Mock API服务（含倍用心系统接口）")
...
async def health_check():
    return {"status": "ok", "service": "beste-mock-api", "version": "2.0.0", "timestamp": datetime.now().isoformat()}
...
@app.get("/api/beiyongxin/bom/query")
...
# 接口2：订单信息查询（●强依赖）
@app.get("/api/beiyongxin/orders/query")
async def byx_order_query(order_no: Optional[str] = Query(None, description="订单号"), date_from: Optional[str] = Query(None, description="开始日期"), date_to: Optional[str] = Query(None, description="结束日期"), customer_code: Optional[str] = Query(None, description="客户编号")):
    ...
    return {"orders": result, "total": len(result), "dependency": "strong"}

# 接口3：订单状态变更推送（●强依赖）- 事件触发
@app.post("/api/beiyongxin/orders/status-change")
async def byx_status_change(payload: dict):
    """[倍用心] 订单状态变更主动推送（事件触发）"""
    order_no = payload.get("order_no", "")
    if not order_no:
        raise HTTPException(status_code=400, detail={"code": "INVALID_ORDER", "message": "订单号不能为空"})
    return {
...
# 接口4：订单交期数据（●强依赖）
@app.get("/api/beiyongxin/orders/delivery")
...
# 接口5：实时库存查询（●强依赖）
@app.get("/api/beiyongxin/inventory/real-time")
async def byx_inventory_query(material_code: str = Query(..., description="物料编码"), warehouse_code: Optional[str] = Query(None, description="仓库编码")):
    if not material_code:
        raise HTTPException(status_code=400, detail={"code": "INVALID_MATERIAL", "message": "物料编码不能为空"})
    return {
...
# 接口6：欠料明细查询（●强依赖）
@app.get("/api/beiyongxin/inventory/shortage")
async def byx_shortage_query(order_no: str = Query(..., description="订单编号/工单编号")):
    """[倍用心] 查询订单欠料明细"""
    if not order_no:
        raise HTTPException(status_code=400, detail={"code": "INVALID_ORDER", "message": "订单编号不能为空"})
    return {
        "order_no": order_no,
        "shortage_details": [
...
# 接口7：呆滞物料清单（●强依赖）
@app.get("/api/beiyongxin/inventory/slow-moving")
...
# 接口8：采购订单查询（●强依赖）
@app.get("/api/beiyongxin/purchase/orders")
...
# 接口9：供应商主数据（●强依赖）
@app.get("/api/beiyongxin/suppliers/master-data")
async def byx_supplier_master(supplier_code: Optional[str] = Query(None), material_type: Optional[str] = Query(None)):
    ...
    return {"suppliers": result, "total": len(result), "dependency": "strong"}

# 接口10：采购交期状态（●强依赖）
@app.get("/api/beiyongxin/purchase/delivery-status")
async def byx_purchase_delivery_status(po_no: Optional[str] = Query(None)):
    ...
    return {"delivery_statuses": statuses, "total": len(statuses), "dependency": "strong"}

# 接口11：历史交易记录（▲部分依赖）
@app.get("/api/beiyongxin/purchase/history")
async def byx_purchase_history(supplier_code: str = Query(..., description="供应商编码"), months: int = Query(12, description="回溯月数")):
    """[倍用心] 获取供应商历史交易记录"""
    if not supplier_code:
        raise HTTPException(status_code=400, detail={"code": "INVALID_SUPPLIER", "message": "供应商编码不能为空"})
    return {
...
# 接口12：产线入库状态（●强依赖）
@app.get("/api/beiyongxin/production/warehouse-in")
...
# 接口13：生产进度（▲部分依赖）
@app.get("/api/beiyongxin/production/progress")
...
# 接口14：产能数据（▲部分依赖）
@app.get("/api/beiyongxin/production/capacity")
...
# 接口15：出库明细（●强依赖）
@app.get("/api/beiyongxin/finance/outbound-detail")
...
# 接口16：收款记录（●强依赖）
@app.get("/api/beiyongxin/finance/payment-records")
...
# 接口17：费用支出（▲部分依赖）
@app.get("/api/beiyongxin/finance/expenses")
...
# 接口18：工时数据（▲部分依赖）
@app.get("/api/beiyongxin/finance/labor-hours")
...
# 接口19：客户信息（●强依赖）
@app.get("/api/beiyongxin/crm/customer-info")
async def byx_crm_customer_info(customer_code: Optional[str] = Query(None)):
    ...
    return {"customers": custs, "total": len(custs), "dependency": "strong"}

# 接口20：跟进记录（▲部分依赖）
@app.get("/api/beiyongxin/crm/follow-up")
...
# 接口21：商机状态（●强依赖）
@app.get("/api/beiyongxin/crm/opportunity")
